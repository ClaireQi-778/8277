...
@NamedQuery(name = "StudentClub.findAll", query = "SELECT sc FROM StudentClub sc LEFT JOIN FETCH sc.studentMembers")
@Inheritance(strategy = InheritanceType.SINGLE_TABLE)
@DiscriminatorColumn(name = "academic", discriminatorType = DiscriminatorType.INTEGER, length = 1)
// FIX: Add Jackson annotations to handle JSON polymorphism
@JsonTypeInfo(
    use = JsonTypeInfo.Id.NAME, 
    include = JsonTypeInfo.As.PROPERTY, 
    property = "entity-type")
@JsonSubTypes({
    @JsonSubTypes.Type(value = Academic.class, name = "academic"),
    @JsonSubTypes.Type(value = NonAcademic.class, name = "non_academic")
})
public class StudentClub extends PojoBase implements Serializable {
	...
	@Column(name = "academic", insertable = false, updatable = false)
