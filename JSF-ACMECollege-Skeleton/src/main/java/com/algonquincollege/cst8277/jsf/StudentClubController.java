...
public class StudentClubController implements Serializable, MyConstants {
    ...
    public void loadStudentClubs() {
        Response response = webTarget
                .register(auth)
                .path(STUDENTCLUB_RESOURCE_NAME)
                .request()
        ...
        if (response.getStatus() == 200) {
            ...
            System.out.println("Failed to load student clubs. Status: " + response.getStatus());
            String errorBody = response.readEntity(String.class);
            System.out.println("Error response: " + errorBody);
            listOfStudentClubs = new java.util.ArrayList<>();
        }
    }
    ...
    public String updateStudentClub(StudentClub studentClub) {
        Response response = webTarget
                .register(auth)
                .path(STUDENTCLUB_RESOURCE_NAME + "/" + studentClub.getId())
                .request()
    ...
    public String deleteStudentClub(int studentClubId) {
        Response response = webTarget
                .register(auth)
                .path(STUDENTCLUB_RESOURCE_NAME + "/" + studentClubId)
                .request()
                .get();
        ...
        if (studentClubToBeDeleted != null) {
            response = webTarget
                    .register(auth)
                    .path(STUDENTCLUB_RESOURCE_NAME + "/" + studentClubToBeDeleted.getId())
                    .request()
                    .delete();
